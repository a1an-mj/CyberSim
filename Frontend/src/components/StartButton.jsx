...
import { Play, Loader2, CheckCheck } from 'lucide-react';

function StartButton({ isRunning = "idle", buttonStatus: externalStatus, onReset }) {
    ...
    <div className="relative">
      <style jsx>{`
        @keyframes rollToLeft {
          ...
          80%  { opacity: 1; }
          100% { transform: translateX(-154px) rotate(-720deg); opacity: 0; }
        }
        @keyframes fadeInLeft {
          0%   { opacity: 0; transform: scale(0.8); }
          100% { opacity: 1; transform: scale(1); }
        }
        .roll-left   { animation: rollToLeft 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
        .fade-in-left { animation: fadeInLeft 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards 0.6s; opacity: 0; }
      `}</style>
      ...
      {isRunning === "idle" && (
        ...
        >
          <span className="text-base font-mono text-zinc-200 font-semibold">Idle</span>
      ...
      {isRunning === 'finished' && (
        <button
          onClick={handleReset}
          className="group bg-gradient-to-b from-zinc-800/70 to-zinc-900/70 backdrop-blur-sm rounded-full px-5 py-2.5 border border-zinc-700 shadow-2xl transition-all duration-300 hover:scale-105 flex items-center gap-3 min-w-[196px]"
        >
          <div className="bg-gradient-to-br from-blue-400 to-blue-500 rounded-full p-2.5 transition-all duration-300 group-hover:scale-110 shadow-lg shadow-blue-500/50">
            <CheckCheck className="w-4 h-4 text-zinc-900" />
          </div>
          <span className="text-base font-mono text-blue-400 font-semibold">Finished</span>
        </button>
      )}
    </div>
  );
